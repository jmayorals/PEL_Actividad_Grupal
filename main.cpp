...
std::mutex mutex_processed_data; // Declarar el mutex globalmente
...
// fs::is_regular_file y fs::is_directory para determinar si la rutarefiere
...
if (fs::is_regular_file(pth)) {
        // Obtener la extensión del archivo y su tamaño
        std::string ext = pth.extension().string();
        std::uintmax_t size = fs::file_size(pth);

        // Bloque crítico para evitar condiciones de carrera en el acceso a processed_data
        {
            std::lock_guard<std::mutex> lock(mutex_processed_data);
            // Actualizar el mapa res con la información del archivo
            res[ext].num_files++;
            res[ext].total_size += size;
        }
    } else if (fs::is_directory(pth)) {
        // Incrementar el contador atómico de subdirectorios
        directory_counter++;
    }
...
for (std::size_t i = 0; i < num_futures; ++i) {
    auto start_idx = i * max_chunk_sz;
    auto end_idx = (i == num_futures - 1) ? paths.size() : (i + 1) * max_chunk_sz;
    auto chunk = std::vector<fs::path>(paths.begin() + start_idx, paths.begin() + end_idx);
    futures.push_back(std::async(std::launch::async, generate_map, std::move(chunk)));
}
...
// [[BLOQUE DE CÓDIGO 3 A IMPLEMENTAR POR EL EQUIPO DE TRABAJO]]
    ...
    std::lock_guard<std::mutex> lock(mutex_processed_data);
    // Fusionar el mapa parcial dentro del mapa principal processed_data
...
for (auto& future : futures) {
    auto partial_map = future.get();
    process_map(partial_map);
}
...
// [[BLOQUE DE CÓDIGO 5 A IMPLEMENTAR POR EL EQUIPO DE TRABAJO]]
    ...
    // Actualizar las variables root_file y root_size
...
"Total", root_file, root_size, directory_counter.load(),
duration.count());
}